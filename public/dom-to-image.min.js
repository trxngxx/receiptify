...
!(function (a) {
		...
		function f(a, b) {
			...
			function d() {
				function c(c) {
					function d(a, b, c) {
						function d(a) {
							const b = a.getPropertyValue('content');
					...
					const e = window.getComputedStyle(a, c),
	...
	function m() {
		...
		function e(a) {
			return new Promise(function (b) {
				for (
					const c = window.atob(a.toDataURL().split(',')[1]), d = c.length, e = new Uint8Array(d), f = 0;
					f < d;
					f++
				)
					e[f] = c.charCodeAt(f);
		...
		function g(a, b) {
			const c = document.implementation.createHTMLDocument(),
			...
			return (
				...
				new Promise(function (c) {
					...
					if (v.impl.options.imagePlaceholder) {
						const i = v.impl.options.imagePlaceholder.split(/,/);
						i && i[1] && (h = i[1]);
					}
				})
			);
		...
		function n(a) {
			for (const b = [], c = a.length, d = 0; d < c; d++) b.push(a[d]);
			return b;
		}
		function o(a) {
		...
		function p(a) {
			const b = r(a, 'border-left-width'),
				c = r(a, 'border-right-width');
			return a.scrollWidth + b + c;
		}
		function q(a) {
			const b = r(a, 'border-top-width'),
				c = r(a, 'border-bottom-width');
			return a.scrollHeight + b + c;
		}
		function r(a, b) {
			const c = window.getComputedStyle(a).getPropertyValue(b);
	...
	function n() {
		...
		function b(a) {
			for (const b, c = []; null !== (b = e.exec(a)); ) c.push(b[1]);
	...
	function o() {
		...
		function b() {
			...
			function c(a) {
				return {
					resolve: function () {
						const b = (a.parentStyleSheet || {}).href;
	...
	function p() {
		...
		function b(c) {
			function d(a) {
				const b = a.style.getPropertyValue('background');
				return b
					? r
							.inlineAll(b)
							.then(function (b) {
